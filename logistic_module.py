...
class Classifier(BaseEstimator, TransformerMixin):
    def __init__(self, classifier='logistic'):
        ...
        self.logreg_C = [ 0.0001, 0.001, 0.01, 0.1]
        # Maximum iterations for solver
        self.max_iter = [1000]
         # Pipeline: scaling + classifier  
        self.pipe = Pipeline(steps=[('scaler', StandardScaler()),
                                    ('classifier', self.create_classifier())])# Add classifier,# Standardize features
    ...
    def create_classifier(self):
        ...
        if self.classifier == 'logistic':
            return LogisticRegression(random_state=42, class_weight='balanced')#penalty='elasticnet',l1_ratio=0.001,solver='saga'
    ...
    def get_grid(self):
        ...
        if self.classifier == 'logistic':
            return {'classifier__C': self.logreg_C,
               # 'classifier__solver': self.solver,
                'classifier__max_iter': self.max_iter}
        else:
            raise ValueError("Invalid classifier option")
    # -----------------------------
    # Train-test split
    ...
    def split_train_test(self, df, y):
        ...
        imbalance_ratio = num_negative_samples / num_positive_samples
    ...
    def grid_train(self, pipe, grid, cv, train_features, train_labels, scoring, metric):
        ...
        grid_search = GridSearchCV(estimator=pipe, param_grid=grid, cv=stratified_cv, verbose=2, n_jobs=-1, scoring=scoring, refit=metric, return_train_score=True, error_score='raise')
        ...
        print(best_grid)
        print('Name steps............', best_grid.named_steps)

        # Access the logistic regression estimator
        logistic_model = best_grid.named_steps['classifier']
        ...
        print("Mean test AUC for each candidate:", grid_search.cv_results_['mean_test_auc'])
        print("Mean train AUC for each candidate:", grid_search.cv_results_['mean_train_auc'])
    ...
    def roc_auc_binary(self, y_test, y_score, class_label):
        ...
        df = pd.DataFrame({'FPR': fpr, 'TPR': tpr, 'roc_auc':roc_auc})
        df.to_csv('ecd_logistic_roc_curve_updated.csv', index=False)
   ...
   #SHAP Summary Plot
   # -------------------------------
    def shap_summary_plot(self, shap_values, X_test):
        """
        Plots SHAP summary plots for model interpretability.

        Parameters:
        shap_values : list or array
            SHAP values (can be multi-class)
        ...
        for i, v in enumerate(list(self.label_binarizer.classes_)):
            plt.title(f'class {v}')
   ...
   # -------------------------------
    def train_evaluate(self, cv, data, y, scoring, metric):
        ...
        # Save Trained Model
        model_path_joblib = r"C:\Users\faith.neema\OneDrive - Aga Khan University\Documents\Longitudinal studies\logistic_regression_trained_with_21_features.joblib"
        model_path_pkl = r"C:\Users\faith.neema\OneDrive - Aga Khan University\Documents\Longitudinal studies\logistic_regression_trained_with_21_model.pkl"
        dump(best_model, model_path_joblib)
        dump(best_model, model_path_pkl)
        ...
        # ROC-AUC Curves
        print('---AUC_ROC for Training Data ---')
        self.roc_auc_binary(y_train, y_prob_train, label_binarizer.classes_)
        print('---AUC_ROC for Testing Data ---Before calibration')
        ...
        prob_true, prob_pred = calibration_curve(y_test,   y_prob_test[:, 1], n_bins=7,strategy='uniform')
        ...
        plt.show()
        print('After calibration')
        prob_true, prob_pred = calibration_curve(y_test,   y_prob_test_cal1, n_bins=13,strategy='uniform')
        ...
        plt.rcParams.update({
            ...
            "pdf.fonttype": 42,
            "ps.fonttype": 42,
            "text.usetex": False            # Font size for y-axis tick labels
        })
        ...
        shap.summary_plot( shap_values_test,
                  X_test, plot_type="bar",max_display=20, plot_size=(15, 10), show=False)
        ...
        plt.suptitle("B. Feature importance plot by LR for off-track development", fontsize=34, y=1.02)

        # Save as separate PDF
        plt.savefig(os.path.join(output_folder, "Log_feature_plot.pdf"), bbox_inches="tight",format='pdf',pad_inches=0)
        #plt.close()
        plt.show()
        shap.summary_plot( shap_values_test,
                          X_test, max_display=20, plot_size=(15, 10), show=False)
        ax = plt.gca()
        ...
        cbar.set_ylabel(cbar.get_ylabel(), fontsize=30)  # Adjust font size and weight as needed, fontweight='bold'

        # Display the updated plot
        #plt.suptitle("B. Beeswarm plot by LR for off-track development", fontsize=24, fontweight='bold', y=1.02)

        #plt.savefig(os.path.join(output_folder, "LR_SHAP_Beeswarm.pdf"), format='pdf',bbox_inches="tight",pad_inches=0)
        #plt.close()
        plt.show()
        #plt.show()
        for i in range(30):
            ...
            ax.set_ylabel(ax.get_ylabel(), fontsize=13)  # Modify size as needed fontweight='bold'
            for label in ax.get_yticklabels():
                #label.set_fontweight('bold')
            ...
            for label in ax.get_xticklabels():
             ...
             # Display the updated plot
            plt.suptitle(f"Figure {i+3}: SHAP Dependence Plot for feature {i}", fontsize=16,  y=1.02)

            #plt.savefig(os.path.join(output_folder, f"Figure{i+3}_SHAP_Dependency1.pdf"), format='pdf',bbox_inches="tight",pad_inches=0)
            #plt.close()
            #plt.show()
